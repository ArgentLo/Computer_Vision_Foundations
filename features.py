...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        X_centered = None  # zero-centered data == (X - X.mean)
        ...
        if method=='eigen':
            e_vecs, e_vals = self._eigen_decomp(X)

        elif method == 'svd':
            e_vecs, e_vals = self._svd(X)

        else:
            print('method: must be "svd" or "eigen"')
        ...
        eig_zero_template = np.zeros_like(e_vecs)
        eig_zero_template[:len(e_vals), :len(e_vals)] = np.diag(e_vals)
        # self.W_pca = np.matmul(e_vecs, self.W_pca)
        self.W_pca = e_vecs
    ...
    def _eigen_decomp(self, X):
        ...
        X_centered = X  # in this example, the input is already centered

        # 1. covariance matrix of X, of shape (D, D)
        cov = np.dot(X_centered.T, X_centered) / (N-1)

        # 2. eigenvalues and eigenvectors of the cov
        e_vals, e_vecs = np.linalg.eig(cov)

        # 3. Sort in descending order
        #    as well as remove Complex Number
        order = np.argsort(e_vals)[::-1]  # descending order (biggest --> lowest)
        e_vals = np.real(e_vals[order])
        e_vecs = np.real(e_vecs[:, order])
    ...
    def reconstruct(self, X_proj):
        ...
        # self.W_pca is (N, n_components). By matrix(n_components, D) --> reconstruct into shape (N, D)
        # since X_proj = np.matmul(X_c, W_pca), to get X_c, just compute the inverse of W_pca

        # the inverse of the projection matrix
        inv_projection = np.linalg.inv(self.W_pca)
        X = np.matmul(X_proj, inv_projection[:n_components, :])
        X += self.mean
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        left_part = np.matmul(inv_scatter_within, scatter_between)
        e_vals, e_vecs = scipy.linalg.eig(left_part)
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            curr_X = X[y==i]
            Xi_mean = np.mean(curr_X, axis=0)
            Xi_centered = curr_X - Xi_mean

            i_scatter = np.matmul(Xi_centered.T, Xi_centered)

            scatter_within += i_scatter
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE############################################################################################
            """
            Here "Compute the covariance matrix as if each class is at its mean" --> do Not (mu_i - mu) for better result
            """
            X_i = temp_X[y == i]
            mu = X_i.mean(axis=0)
            temp_X[y == i] = mu
    ...
    def transform(self, X, n_components):
        ...
        W_lda = self.W_lda[:, :n_components]

        X_proj = np.matmul(X, W_lda)
