...
def energy_function(image):
    ...
    # computer the gradient by np.gradient
    dy = np.absolute(np.gradient(img)[0])  # abs values of gradient wrt Y-axis
    dx = np.absolute(np.gradient(img)[1])  # abs values of gradient wrt X-axis

    out = dy + dx  # the energy
...
def compute_cost(image, energy, axis=1):
    ...
    # Top to Bottom: Compute the Cost (Remember, its Aggregating along - the bottom costs are great numbers)
    ...
    for i in range(1, H):
        for j in range(W):
            ...
            min_cost = np.amin(cost[i-1, left:right])
            cost[i, j] = energy[i, j] + min_cost

            # Paths

            if (j-1>=0) and min_cost == cost[i - 1, j - 1]:
                paths[i, j] = -1
            elif (j+2<=W-1) and min_cost == cost[i - 1, j + 1]:
                paths[i, j] = +1
...
def backtrack_seam(paths, end):
    ...
    # Initialization
    seam[H-1] = end  # the last row would be assigned np.argmin(cost[-1]) --> the col_idx of the least Cost

    ### YOUR CODE HERE##################################################################################################

    for i in range(H-2, -1, -1):
        last_col_idx = seam[i+1]
        seam[i] = last_col_idx + paths[i+1, last_col_idx]
...
def remove_seam(image, seam):
    ...
    if len(image.shape) == 2:
        image = np.expand_dims(image, axis=2)  # If 2D, then now becomes (H,W)-->(H,W,C)=(H,W,1)
    ...
    for i in range(H):
        out[i, 0:seam[i]] = image[i, 0:seam[i]]
...
def reduce(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(int(W-size)):
        ...
        energy = efunc(out)
        cost, paths = cfunc(..., energy)
        seam = backtrack_seam(paths, np.argmin(cost[-1]))
        out = remove_seam(out, seam)

        if (100*i/(int(W-size))) % 5 == 0:
            print('Completed %d' % ((100*i/(int(W-size)))), '%')
...
def duplicate_seam(image, seam):
    ...
    for i in range(H):
        ...
        col = seam[i]
        if col == 0:
            out[i, col:(col+2)] = image[i, col]
            out[i, (col+2):] = image[i, (col+1):]
        elif col == W-1:
            out[i, :col] = image[i, :col]
            out[i, col] = image[i, col]
            out[i, col+1] = image[i, col]
        else:
            out[i, :col] = image[i, :col]
            out[i, col:(col+2)] = image[i, col]
            out[i, (col + 2):] = image[i, (col + 1):]
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    #    [[0, 1, 0, 2],
    #     [1, 0, 2, 0],
    #     [1, 0, 0, 2]]  # 1 denotes the first seam(top to bottom); 2 denotes the second seam
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(int(size-W)):
        ...
        idx = np.where(seams==i+1)
        seam = idx[1]

        # enlarge both the Image and the Seams
        out = duplicate_seam(out, seam)

        new_seams = np.array([])
        for j in range(H):
            new_row = np.insert(seams[j], obj=seam[j], values=(i+1))
            new_seams = np.append(new_seams, new_row)
        seams = new_seams.reshape((H, W+i+1))
        ...
        if (100*i/(int(size-W))) % 15 == [0, 1, 2]:
            print('Completed %d' % ((100*i/(int(size-W)))), '%')
