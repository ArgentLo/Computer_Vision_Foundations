...
def convert_to_grey_scale(image):
    ...
    ### YOUR CODE HERE
    # Using Numpy to convert -- PIL.ImageOps.grayscale(image) is a better choice
    img = image.copy()
    out = np.dot(img[:, :, :], [0, 0.5, 0.5])
...
def rgb_decomposition(image, channel):
    ...
    if channel == 'R':
        ...
        out = np.dot(img[:, :, :], [0.5, 0, 0.5])

    elif channel == 'B':
        out = np.dot(img[:, :, :], [0.5, 0.5, 0])
...
def lab_decomposition(image, channel):
    ...
    if channel == 'L':
        out = np.dot(lab[:, :, :], [1, 0, 0])

    elif channel == 'A':
        out = np.dot(lab[:, :, :], [0, 1, 0])

    elif channel == 'B':
        out = np.dot(lab[:, :, :], [0, 0, 1])
...
def hsv_decomposition(image, channel='H'):
    ...
    if channel == 'H':
        out = np.dot(hsv[:, :, :], [1, 0, 0])

    elif channel == 'S':
        out = np.dot(hsv[:, :, :], [0, 1, 0])

    elif channel == 'V':
        out = np.dot(hsv[:, :, :], [0, 0, 1])
