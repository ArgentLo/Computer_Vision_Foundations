...
def compute_distances(X1, X2):
    ...
    """
    Euclidean distance: sqrt_sum(x-y) = sqrt(x^2 + y^2 -2xy)
    """
    X1_sum_of_square = np.sum((X1**2), axis=1)  # shape=(M,)
    X2_sum_of_square = np.sum((X2**2), axis=1)  # shape=(N,)
    dot_prodcut = np.dot(X1, X2.T)  # shape=(M,N)

    X1_ss = np.stack([X1_sum_of_square] * N, axis=1)  # horizontally stack, shape=(M,N)
    X2_ss = np.stack([X2_sum_of_square] * M, axis=0)  # vertically stack, shape=(M,N)

    # Distance = sum of 3 matrices and take square_root

    sum = X1_ss + X2_ss - (2*dot_prodcut)
    dists = np.sqrt(sum)
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # find the first K closest idx
        k_idx = np.argsort(dists[i])[:k]
        # find these labels
        closest_y = y_train[k_idx]

        # find the most common label within the set of "closest_y"
...
def split_folds(X_train, y_train, num_folds):
    ...
    # Split the array X_train, y_train sets.
    # IMPORTANT: - we assume they are uniformly shuffled.

    # both with shape(folds_idx, N,..)
    X_train_splitted = np.array(np.array_split(X_train, num_folds, axis=0))
    y_train_splitted = np.array(np.array_split(y_train, num_folds, axis=0))

    for i in range(num_folds):

        # Boolean array to separate into Training and Validation
        #         and reshape back to 2-dimension
