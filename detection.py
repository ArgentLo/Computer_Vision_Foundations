...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    Args:
        ...
        base_score: hog representation of the object you want to find --> e.g. Average_Face of shape(218, 178)
    ...
    # pad the image before sliding the window
    ...
    for i in range(0, H+1, stepSize):
        for j in range(0, W+1, stepSize):
            ...
            curr_hog = feature.hog(window, pixels_per_cell=(pixel_per_cell, pixel_per_cell))

            score = np.matmul(curr_hog.T, base_score)

            response_map[i // stepSize, j // stepSize] = score

            if score > max_score:
                max_score = score
                # back to the un-padded image
                maxr = i - winH // 2
                maxc = j - winW // 2
...
def pyramid(image, scale=0.9, minSize=(200, 100)):
    ...
    while current_scale * image.shape[0] > minSize[0] and \
            current_scale * image.shape[1] > minSize[1]:
...
def pyramid_score(image,base_score, shape, stepSize=20, scale = 0.9, pixel_per_cell = 8):
    ...
    Args:
        ...
        base_score: the hog representation of the object you want to detect --> e.g. Average_Face of shape(218, 178)
    ...
    for curr_scale, img in images:

        score, row, col, response_map = sliding_window(img, base_score, stepSize, shape, pixel_per_cell=pixel_per_cell)
        if score > max_score:
            max_score = score
            maxr = row
            maxc = col
...
def compute_displacement(part_centers, face_shape):
    '''
        ...
        Note that the heatmap pixels will be shifted
        by an int, so mu is an int vector.
    
    Args:
        part_centers: np array of shape (n,2) containing centers 
                      of one part in each image (#images=n)
        face_shape: (h,w) that indicates the shape of a face
    Returns:
        mu: (1,2) vector --> the distance from the main center,
                             i.e the center of the face to the center of the part.
        sigma: (1,2) vector
    ...
    # (face_center) - (curr_part_position_in_each_img)
...
def shift_heatmap(heatmap, mu):
    ...
    # normalize the heatmap to make sure that all the values are not larger than 1
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for heatmap, sigma in zip(heatmaps, sigmas):
        ...
        # using the skimage.feature.gaussian()
